...
RETIRED_PLUGIN_DIRECTIVE_RE = re.compile(r"Use @asynchronia(?: plugin| [A-Za-z0-9-]+)\.")
MERGE_MARKERS = ("<<<<<<<", "=======", ">>>>>>>")
...
def validate_file(
    ...
    *,
    ...
    if schema_name == "03-codex-task.md" and enforce_active_codex_rules:
        if RETIRED_PLUGIN_DIRECTIVE_RE.search(text):
            errors.append(
                f"{path}: active executable Codex prompt contains a retired external-plugin directive"
            )
