...
def patch_boot(path: Path) -> None:
    boot = path.read_text(encoding="utf-8")
    resume_anchor = "\n".join([
    ...
    boot = replace_once(boot, resume_anchor, resume_block, f"{path} resume boot")
    ...
    boot = replace_once(boot, fresh_anchor, fresh_block, f"{path} fresh boot")
...
patch_boot(SOURCE_BOOT)
patch_boot(DOCS_BOOT)
print("PASS_STAGE7_RUNTIME_PATCH_APPLIED_INDEPENDENT_BOOTS")
