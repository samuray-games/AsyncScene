...
def installed_plugin_root(directory: Path, version: str = "1.0.18", name: str = "asynchronia") -> Path:
...
class ModelSelectorAuthorityTests(unittest.TestCase):
    ...
    def test_read_only_canary_short_circuits_without_mutation_contract(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("manifest version: 1.0.18", start.stdout)
    ...
    def test_active_plugin_version_surfaces_agree(self) -> None:
        ...
        self.assertEqual(entry["version"], "1.0.18")
        self.assertIn('PLUGIN_VERSION = "1.0.18"', (ROOT / "plugins/asynchronia/model_selector.py").read_text(encoding="utf-8"))
        self.assertIn('PLUGIN_VERSION = "1.0.18"', (ROOT / "plugins/asynchronia/model_selector_runtime.py").read_text(encoding="utf-8"))
        self.assertIn("Asynchronia 1.0.18", (ROOT / "plugins/asynchronia/skills/model-selector/SKILL.md").read_text(encoding="utf-8"))
        self.assertIn('EXPECTED_VERSION = "1.0.18"', (ROOT / "tools/validate-asynchronia-auto-model-preflight.py").read_text(encoding="utf-8"))
    ...
    def test_read_only_runtime_version_comes_from_manifest_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            state_dir = Path(directory) / "state"
            plugin_root = installed_plugin_root(Path(directory), version="1.0.18")
            task_file = Path(directory) / "task.json"
            task_file.write_text(json.dumps(read_only_task()), encoding="utf-8")
            command = [sys.executable, "tools/run-asynchronia-model-preflight.py"]
    ...
    def test_cheapest_rejected_pair_and_rendered_output_match_lowest_frontier_rejection(self) -> None:
        ...
        expected = report.cheapestRejected
