...
class ModelSelectorTests(unittest.TestCase):
    ...
    def test_mutation_state_machine_requires_inventory_ok_then_continue(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("exact next response: INVENTORY_OK", start.output)
            with self.assertRaises(AuthorizationError):
            ...
            self.assertIn("instruction: select the recommended pair in the Codex UI", waiting.output)
            self.assertIn("mutation authorization: not yet authorized", waiting.output)
