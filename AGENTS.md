...
- dynamically locate the current ChatGPT Desktop renderer mechanism in `app.asar` by `list-models-for-host`, never a permanent chunk filename or hash;
- invoke `list-models-for-host` with `hostId`, `includeHidden: true`, `cursor`, and `limit`, following all returned cursor pages;
- record ChatGPT Desktop version, bundle path, located renderer asset, discovery timestamp, source mechanism, raw inventory hash, and filtered inventory hash;
- preserve exact returned `model`, `hidden`, `isDefault`, `defaultReasoningEffort`, and `supportedReasoningEfforts` values and returned order;
- retain hidden entries in complete inventory but include them in the selectable matrix only when current host `use_hidden_models` is explicitly enabled;
- use current `available_models`, `use_hidden_models`, and `default_model` settings as host evidence;
- use a previously user-confirmed snapshot, app-server evidence, or historical cache hint only after the live query is unavailable and label it as fallback evidence, never as live UI authority;
- return `BLOCKED_MODEL_INVENTORY_UNAVAILABLE` when neither live authority nor a confirmed fallback exists;
- preserve model and effort identifiers exactly without adding, renaming, normalizing, translating, or inferring options;
- preserve returned identifiers exactly without normalizing labels or effort names;
...
- fail closed when inventory discovery, effort coverage, pagination, provenance, or task scope is incomplete.
...
Exact same-thread user-confirmed inventory is per-run fallback evidence only. It must not be copied into repository memory as a static model catalog, must not create a permanent fallback list, and must not authorize a different task, thread, surface, or later launch.
