...
Repair the Asynchronia `model-selector` so every model preflight discovers the current official Codex catalog, surface-specific controls, and current Codex rate card at run time instead of trusting a static repository whitelist or fixed pair count.
...
- The model catalog is dynamic and must be verified against current official OpenAI sources during every preflight.
...
- The blocking same-thread `CONTINUE` state machine remains unchanged.
- Plugin source version, marketplace version, and validator expectation move together from `1.0.7` to `1.0.8`.
- The one-time bootstrap repair uses the ChatGPT-authored selection `GPT-5.6 Sol` with `High` reasoning. The stale selector must not select a model for its own repair.
...
- Work must apply only the exact ChatGPT-authored patch in `02-work-plan.md`.
...
- `AGENTS.md` contains a dynamic discovery rule and no active static model whitelist or fixed pair count.
- `model-selector/SKILL.md` contains the exact replacement supplied by ChatGPT.
- The selector verifies official model and rate-card sources for each preflight.
...
- The same-thread `CONTINUE` gate and post-continue scope revalidation remain intact.
- The validator rejects the old static whitelist, fixed pair counts, missing official sources, and `Ultra` misclassification.
...
Apply the exact patch in `02-work-plan.md` to `infra/model-selector-live-catalog-20260712`, run every required validation, commit, push, refetch the branch, and return a factual evidence report. Do not create a Codex task and do not begin the bridge reset.