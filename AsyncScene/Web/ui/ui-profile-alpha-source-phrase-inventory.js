  ...
  const parseRows = (text) => String(text || "")
    ...
    .map((line) => {
      ...
      return Object.freeze({
        id: `TXT_${match[1]}`,
        profile: parseRowValue(match[2].trim()),
        category: parseRowValue(match[3].trim()),
        surface: parseRowValue(match[4].trim()),
        key: parseRowValue(match[5].trim()),
        currentText: parseRowValue(match[6])
      });
    })
    .filter(Boolean);

  const RAW_ROWS = String.raw`TXT_0001 | shared | start_screen | start_screen_title | start_title | AsyncScene
...
TXT_0064 | genz | battle | battle_results | battle_win | Победа
TXT_0065 | genz | battle | battle_results | battle_loss | Поражение
TXT_0066 | genz | battle | battle_results | battle_draw | Толпа решает
...
TXT_0164 | shared | toast | events_vote | vote_not_enough_points | Не хватает 💰.`;
  ...
  const entries = Object.freeze(parseRows(RAW_ROWS));
  const metadata = Object.freeze({
    inventoryId: "UI_PROFILE_ALPHA_SOURCE_PHRASE_INVENTORY",
    stage: "4-alpha",
    step: "2.2",
    mode: "source_inventory_only",
    totalEntries: 164,
    ...
    buildTag: "build_2026_06_18_step4_alpha_profile_step2_2_source_phrase_inventory_v1",
    commit: "step4_alpha_profile_step2_2_source_phrase_inventory_v1",
    smokeVersion: "alpha_step_2_2_source_phrase_inventory_v20260618_001"
  });

  const requiredCoverageGroups = Object.freeze([
    "start_screen",
  ...
  const newFeatureCoverage = Object.freeze([
    "crowd_voting",
    "cop_flow",
    "respect_flow",
    "rematch",
  ...
  window.UI_PROFILE_ALPHA_SOURCE_PHRASE_INVENTORY = manifest;
  window.Game.UI_PROFILE_ALPHA_SOURCE_PHRASE_INVENTORY = manifest;
  if (!window.Game.__DEV) window.Game.__DEV = {};
  window.Game.__DEV.alphaSourcePhraseInventoryManifest = manifest;
