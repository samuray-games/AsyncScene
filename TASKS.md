...
## 2026-06-18 — Step 3 Boomer profile, step 3.4 new-feature lexical coverage smoke fix2
- Status: READY_FOR_RUNTIME_SMOKE only; Safari/runtime PASS is not claimed.
...
- Added install-time visibility markers for the Safari-exported aliases `Game.__DEV.smokeBoomerNewFeatureCoverageStep34Fix1Once()` and `Game.__DEV.smokeBoomerNewFeatureCoverageStep34Fix2Once()` in `AsyncScene/Web/dev/dev-checks.js` and `docs/dev/dev-checks.js`.
- Build tag: `build_2026_06_18_step3_4_boomer_new_feature_coverage_smoke_fix2_v1`.
