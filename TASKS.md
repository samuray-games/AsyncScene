- Step 4 [7] second smoke exception fix: `Game.__DEV.smokeZoomerNewFeaturesTermsOnce()` was still throwing `Can't find variable: collectTextFileCandidates` because that helper was never defined in the smoke scope. Both served dev-check bundles now route the smoke through the existing `resolveDocCandidates(...)` + `fetchFirst(...)` helpers already present in the same file, and the served identity markers were refreshed to `build_2026_06_05_ak` / `d7fb793` in both app/docs entrypoints. Scope stayed locked: no terminology changes, no UI text changes, no gameplay changes, no economy changes, and no Console.txt usage. Runtime PASS is not claimed; Safari must rerun `Game.__DEV.smokeZoomerNewFeaturesTermsOnce()`.
