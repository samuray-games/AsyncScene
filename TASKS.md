...
- Step 4 [5] error terminology only: tightened the Step 4 [4] status-smoke error copy so the remaining failures read as short, direct, one-line guidance instead of bureaucratic labels. The smoke logic, buttons, statuses, hints, gameplay, and error behavior were not changed; only error texts and identity markers were refreshed to `build_2026_06_05_ac` / `a58c803`, with `Game.__DEV.smokeZoomerErrorTermsOnce()` still the required Safari smoke.
