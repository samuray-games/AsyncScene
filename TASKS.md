- Step 4 [7] self-contained smoke fix: `Game.__DEV.smokeZoomerNewFeaturesTermsOnce()` was still failing in Safari because the previous wiring fix replaced one missing helper with another and still depended on `resolveDocCandidates(...)` being in scope. Both served dev-check bundles now make the Step 4 [7] smoke self-contained by building its own candidate URL list and synchronous XHR fetch loop inside the function, with no references to `fetchTextFromCandidates`, `collectTextFileCandidates`, `resolveDocCandidates`, or any other external doc helper. Served identity markers were refreshed to `build_2026_06_05_al` / `ad41fff`, matching the current short git hash. Scope stayed locked: no terminology changes, no UI text changes, no gameplay changes, no economy changes, and no Console.txt usage. Runtime PASS is not claimed; Safari must rerun `Game.__DEV.smokeZoomerNewFeaturesTermsOnce()`.
