- Step 4 [4] status inventory fix: corrected the real `Game.__DEV.smokeZoomerStatusTermsOnce()` inventory filter to read `entry.source.file` / `entry.source.path` from `collectZoomerTermsInventoryEntries()` instead of the nonexistent flat `entry.file` / `entry.path` fields. This is the actual reason Safari reported `statusEntries: []` while the runtime strings were present.
- Step 4 [4] Safari smoke still stays status-only: it now collects the three runtime status inventory entries `Передача недоступна`, `Статус передачи недоступен`, and `Можно передать` from `trainingControls.status.*` and keeps buttons, errors, hints, gameplay logic, transfer mechanics, and status behavior unchanged.
- Step 4 [4] identity sync: refreshed both served runtime entrypoints and both dev-check bundles to `build_2026_06_05_x` / `1b64743`, and bumped the served dev-checks cache-bust to `step4-4-zoomer-status-terms-x`.
