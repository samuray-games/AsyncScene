- Step 4 [4] status terminology only: rewrote only the training status texts to direct action/result wording, changing `Передача недоступна.`, `Статус недоступен.`, and abstract ready-state `доступно` to `Передача недоступна`, `Статус передачи недоступен`, and `Можно передать` in both runtime UI menu files. Buttons, errors, hints, gameplay logic, and status behavior were not changed.
- Step 4 [4] Safari smoke: added `Game.__DEV.smokeZoomerStatusTermsOnce()` in both dev-check bundles, scoped it to the Step 4 inventory entries for `trainingControls.status.*`, required `ok:true`, empty `failures`/`forbiddenRemaining`/`missingCoverage`/`failedChecks`, and identity fields `buildTag`, `commit`, and commit-unique `smokeVersion`. Refreshed the served dev-checks cache-bust to `step4-4-zoomer-status-terms-v`.
- Step 4 [4] identity sync: aligned both served runtime entrypoints and both dev-check bundles to `build_2026_06_05_w` / `cb519db` so the Safari smoke returns the Step 4 [4] commit marker instead of older Step 4 [2]/[3] hashes.
