...
`
  ...
  const installBoomerDiffStep15SoftVerbsDocTableOnceSmokeViaData = () => {
    if (typeof root.__DEV.smokeBoomerDiffStep15SoftVerbsDocTableOnce === "function") return;
    const smokeVersion = "step1_5_boomer_soft_verbs_doc_table_v20260616_001";
    const expectedRows = [
      { id: "TXT_0019", from: "Погнали", to: "Начать" },
      { id: "TXT_0020", from: "Снести выбор", to: "Сбросить выбор" },
      { id: "TXT_0038", from: "Сдать {name}: +2💰.", to: "Сообщить о {name}: +2💰." },
      { id: "TXT_0039", from: "Коп: {name} сдан, +2💰.", to: "Коп: информация по {name} принята, +2💰." },
      { id: "TXT_0041", from: "{name} зовёт на реванш.", to: "{name} предлагает реванш." },
      { id: "TXT_0042", from: "Свалить за 1💰.", to: "Уйти за 1💰." },
      { id: "TXT_0047", from: "Свалить: -{escapeCost}💰.", to: "Уйти: -{escapeCost}💰." },
      { id: "TXT_0050", from: "{attackerName} [{attackerInf}] бросил вызов.", to: "{attackerName} [{attackerInf}] начал конфликт." },
      { id: "TXT_0070", from: "Свалить: {X}", to: "Уйти: {X}" },
      { id: "TXT_0073", from: "Введи точный ник.", to: "Введите точный ник." },
      { id: "TXT_0079", from: "Понял. Проверяю.", to: "Принято. Проверяю." },
      { id: "TXT_0080", from: "Принял. Разберусь.", to: "Принято. Проверю." },
      { id: "TXT_0083", from: "Проверка сошлась. Вмешался.", to: "Проверка подтвердилась. Меры приняты." },
      { id: "TXT_0084", from: "Проверка сошлась. Занялся.", to: "Проверка подтвердилась. Работа начата." },
      { id: "TXT_0087", from: "Ответь: кто?", to: "Ответьте: кто?" },
      { id: "TXT_0088", from: "Ответь: где?", to: "Ответьте: где?" },
      { id: "TXT_0089", from: "Ответь: о ком?", to: "Ответьте: о ком?" },
      { id: "TXT_0090", from: "Ответь: да или нет?", to: "Ответьте: да или нет?" }
    ];
    const forbiddenTokens = ["опасно", "нельзя", "ты должен"];
    const readText = (fileName) => {
      const candidates = [fileName, `docs/${fileName}`];
    ...
    root.__DEV.smokeBoomerDiffStep15SoftVerbsDocTableOnce = function smokeBoomerDiffStep15SoftVerbsDocTableOnce() {
      const result = {
        ok: false,
        buildTag: (typeof window !== "undefined" && window.__BUILD_TAG__) || root.__DEV.buildTag || null,
        commit: (typeof window !== "undefined" && window.__COMMIT__) || root.__DEV.commit || null,
        smokeVersion,
        docPresent: false,
        softVerbsSectionPresent: false,
        copyTablePresent: false,
        expectedRows: expectedRows.length,
        actualRows: 0,
        missingRows: [],
        referencesMillennialDelta: false,
        softVerbRulePresent: false,
        sharpToCalmRulePresent: false,
        slangToNeutralRulePresent: false,
        politeImperativeRulePresent: false,
        noMechanicsChangeRulePresent: false,
        noStandaloneBoomerProfile: false,
        forbiddenToTextHits: [],
        runtimeVisibleCopyUnchanged: false,
      ...
      try {
        const docRes = readText("UI_PROFILE_BOOMER_DIFF.md");
        const runtimeRes = readText("AsyncScene/Web/data.js");
        result.docPresent = !!docRes.ok;
        const doc = docRes.text || "";
        result.softVerbsSectionPresent = doc.includes("## SOFT VERBS");
        result.copyTablePresent = doc.includes("## EXACT SOFT VERB COPY TABLE");
        result.referencesMillennialDelta = doc.includes("delta from UI_PROFILE_MILLENNIAL");
        result.softVerbRulePresent = doc.includes("Boomer verb language is a delta from UI_PROFILE_MILLENNIAL.");
        result.sharpToCalmRulePresent = doc.includes("sharp verbs become calm verbs");
        result.slangToNeutralRulePresent = doc.includes("slang commands become neutral actions");
        result.politeImperativeRulePresent = doc.includes("player-facing imperatives become polite forms");
        result.noMechanicsChangeRulePresent = doc.includes("mechanic meaning does not change");
        result.noStandaloneBoomerProfile = doc.includes("no standalone boomer profile is created");
        result.actualRows = expectedRows.filter((row) => doc.includes(row.id)).length;
        result.missingRows = expectedRows.filter((row) => !doc.includes(row.id) || !doc.includes(`FROM:\n${row.from}`) || !doc.includes(`TO:\n${row.to}`)).map((row) => row.id);
        result.forbiddenToTextHits = expectedRows.filter((row) => forbiddenTokens.some((token) => row.to.includes(token))).map((row) => row.id);
        result.runtimeVisibleCopyUnchanged = !!runtimeRes.ok && !runtimeRes.text.includes("## SOFT VERBS");
        if (!result.docPresent) fail("doc_missing", null);
        if (!result.softVerbsSectionPresent) fail("soft_verbs_section_missing", null);
        if (!result.copyTablePresent) fail("copy_table_missing", null);
        if (result.expectedRows !== 39) fail("expected_rows_mismatch", result.expectedRows);
        if (result.actualRows !== 39) fail("actual_rows_mismatch", result.actualRows);
        if (result.missingRows.length) fail("missing_rows", result.missingRows.slice());
        if (!result.referencesMillennialDelta) fail("references_millennial_delta_missing", null);
        if (!result.softVerbRulePresent) fail("soft_verb_rule_missing", null);
        if (!result.sharpToCalmRulePresent) fail("sharp_to_calm_rule_missing", null);
        if (!result.slangToNeutralRulePresent) fail("slang_to_neutral_rule_missing", null);
        if (!result.politeImperativeRulePresent) fail("polite_imperative_rule_missing", null);
        if (!result.noMechanicsChangeRulePresent) fail("no_mechanics_change_rule_missing", null);
        if (!result.noStandaloneBoomerProfile) fail("no_standalone_boomer_profile_missing", null);
        if (result.forbiddenToTextHits.length) fail("forbidden_tokens", result.forbiddenToTextHits.slice());
        if (!result.runtimeVisibleCopyUnchanged) fail("runtime_visible_copy_changed", null);
    ...
    root.Dev.smokeBoomerDiffStep15SoftVerbsDocTableOnce = root.__DEV.smokeBoomerDiffStep15SoftVerbsDocTableOnce;
  };

  installBoomerDiffStep15SoftVerbsDocTableOnceSmokeViaData();
