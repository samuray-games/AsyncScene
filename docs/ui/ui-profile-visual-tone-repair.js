...
(function installStage6FinalPresentationV4() {
  ...
  const CONTROL_COPY = Object.freeze({
    ...
    zoomer: Object.freeze({
      chatPlaceholder: "Что скажешь?",
      chatSend: "Кинуть",
      dmPlaceholder: "В личку",
      dmSend: "Кинуть",
      dmHeader: "ЛС",
    ...
    alpha: Object.freeze({
      ...
      attackBadge: "Ход",
      defenseBadge: "Ответ",
      opponentArgLabel: "Ход соперника",
  ...
  const TEXT_OVERRIDES = Object.freeze({
    boomer: Object.freeze({
      ...
      result_draw: "Ничья.",
      vote_majority: "Вы поддержали большинство.",
      vote_minority: "Вы оказались в меньшинстве.",
    ...
    genX: Object.freeze({
      ...
      result_win: "Нормально разобрались.",
      result_loss: "Вот это облом.",
      result_draw: "Разошлись по нулям.",
      vote_majority: "Ты оказался с большинством.",
      vote_minority: "Ты остался в меньшинстве.",
    ...
    millennial: Object.freeze({
      ...
      battle_action_report: "Пожаловаться",
      result_win: "Победа. Можно выдохнуть.",
      result_loss: "Ну всё, приехали.",
      result_draw: "Ничья. Никто не победил, зато все устали.",
      vote_majority: "Ты с большинством. Сегодня демократия сработала.",
      vote_minority: "Ты в меньшинстве. Бывает.",
    ...
    zoomer: Object.freeze({
      ...
      result_win: "W. Разнесли.",
      result_loss: "Нас вынесли 💀",
      result_draw: "По нулям.",
      vote_majority: "Ты с большинством. W.",
      vote_minority: "Ты против толпы 💀",
    ...
    alpha: Object.freeze({
      ...
      result_loss: "L 💀",
      result_draw: "ничья =",
      vote_majority: "большинство W",
      vote_minority: "в меньшинстве 💀",
      dm_empty: "личка 0",
  ...
  function startupStatValueAnchor(kind) {
    const chip = document.querySelector(`[data-profile-stat="${kind}"]`);
    if (!chip) return null;
    const id = kind === "points" ? "mePoints" : kind === "rep" ? "meRep" : "";
    const target = (id && chip.querySelector(`#${id}`)) || chip.querySelector(".pointsValue") || chip.querySelector(".statValue");
    if (!target || !target.getBoundingClientRect) return null;
    const r = target.getBoundingClientRect();
    return r.width > 0 && r.height > 0 ? target : null;
  }
